...
class CoreDataManager {
    ...
    private lazy var managedObjectModel: NSManagedObjectModel = {
        let modelURL = Bundle.main.url(forResource: CoreData.dbName, withExtension: "momd")!
        return NSManagedObjectModel(contentsOf: modelURL)!
    }()
    
    private lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        let coordinator = NSPersistentStoreCoordinator(managedObjectModel: self.managedObjectModel)
        let url = self.applicationDocumentsDirectory.appendingPathComponent("\(CoreData.dbName).sqlite")
        var failureReason = MessageConstant.failureReason
        do {
            ...
            dict[NSLocalizedDescriptionKey] = MessageConstant.dictSavedFailed as AnyObject?
            dict[NSLocalizedFailureReasonErrorKey] = failureReason as AnyObject?
            
            dict[NSUnderlyingErrorKey] = error as NSError
            let wrappedError = NSError(domain: "YOUR_ERROR_DOMAIN", code: 9999, userInfo: dict)
            AppLog.debug("\(MessageConstant.unresolvedError) \(wrappedError), \(wrappedError.userInfo)")
            abort()
        }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        container.loadPersistentStores(completionHandler: { (storeDescription, error) in
            if let error = error as NSError? {
                AppLog.severe("\(MessageConstant.unresolvedError) \(error), \(error.userInfo)")
                fatalError("\(MessageConstant.unresolvedError) \(error), \(error.userInfo)")
            }
        })
        return container
    }()
    ...
    func saveContext () {
        if managedObjectContext.hasChanges {
            do {
                ...
                AppLog.debug("\(MessageConstant.unresolvedError) \(nserror), \(nserror.userInfo)")
                abort()
            }
        }
    }
    ...
    func fetch(entityName: NSString) -> [NSManagedObject] {
        ...
        do {
            ...
            AppLog.error("\(MessageConstant.couldFetch) \(error), \(error.userInfo)")
        }
